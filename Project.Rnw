...
Figure~\ref{fig:E_ret} depicts the daily excess returns of the S\&P500 in the stated time period. For visual inspection, we observe a mean of approximately zero, in fact it is \Sexpr{round(mean(E_ret),4)}, while the volatility varies significantly, especially during the financial crisis.
...
%
% \caption{\label{tab:SumStatsFact}Summary statistics of the two factors used for predicting excess returns.}
...
library(xtable)
print(xtable(summary(Factors), caption="Summary statistics of the factors used for predicting excess returns.", label = "tab:SumStatsFact"), booktabs=TRUE, tabular.environment='longtable', caption.placement="top")
@
